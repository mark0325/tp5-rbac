...
class Rbac
{
     ...
     * @var string 用户角色对应表
     */
    private $adminRoleTable = "admin_role";
    ...
    public function __construct()
    {
        ...
        if (!empty($rbacConfig)) {
            ...
            isset($rbacConfig['admin_role']) && $this->adminRoleTable = $rbacConfig['admin_role'];
            isset($rbacConfig['role_permission']) && $this->rolePermissionTable = $rbacConfig['role_permission'];
            isset($rbacConfig['admin']) && $this->adminTable = $rbacConfig['admin'];
    ...
    public function setConfig($config)
    {
        if (!empty($config) && is_array($config)) {
            ...
            isset($config['admin_role']) && $this->adminRoleTable = $config['admin_role'];
            isset($config['role_permission']) && $this->rolePermissionTable = $config['role_permission'];
            isset($config['admin']) && $this->adminTable = $config['admin'];
        }
    }
    ...
    public function createRole(array $data = [])
    {
        ...
        $nestedObj = Db::name($this->roleTable);
        return $nestedObj->insert($data);
    }
     ...
     * 根据id获取角色
     * @param $id
     * @return array|false|\PDOStatement|string|\think\Collection|\think\Model|null
     * @throws \Exception
     */
    ...
    public function assignUserRole($userId, array $role = [])
    {
        ...
        return Db::name($this->adminRoleTable)->insertAll($userRole);
    }
    ...
    public function delUser($id)
    {
        ...
        $dbObj = Db::name($this->adminTable);
        if ($dbObj->where($dbObj->getPk(), $id)->delete() === false) {
            Db::rollback();
            return false;
        }

        if (Db::name($this->adminRoleTable)->where('user_id', $id)->delete() === false) {
            Db::rollback();
            return false;
        }
    ...
    public function createUser(array $data = [])
    {
        ...
        return Db::name($this->adminTable)->insert($data);
    }
    ...
    public function cachePermission($id)
    {
        ...
        $permission = Db::name($this->permissionTable)
            ...
            ->join("{$this->adminRoleTable} ur", "rp.role_id = ur.role_id")
